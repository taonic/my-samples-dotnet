...
public class ContextPropagationInterceptor<T> : IClientInterceptor, IWorkerInterceptor
{
    ...
    public ContextPropagationInterceptor(
        ...
        string headerKey = "__my_metadata")
    {
    ...
    private void SetDiagnosticActivityTags(Activity? activity)
    {
        if (context.Value is Dictionary<string, string> dict)
        {
    ...
    private class ContextPropagationClientOutboundInterceptor : ClientOutboundInterceptor
    {
        ...
        {
            root.SetDiagnosticActivityTags(Activity.Current);
            return base.StartWorkflowAsync<TWorkflow, TResult>(input with { Headers = root.HeaderFromContext(input.Headers) });
        }
    ...
    private class ContextPropagationWorkflowInboundInterceptor : WorkflowInboundInterceptor
    {
        ...
        {
            return root.WithHeadersApplied(Workflow.Info.Headers, () =>
            {
                root.SetDiagnosticActivityTags(WorkflowDiagnosticActivity.Current?.Activity);
    ...
    private class ContextPropagationWorkflowOutboundInterceptor : WorkflowOutboundInterceptor
    {
        ...
        {
            Console.WriteLine(WorkflowDiagnosticActivity.Current?.Activity?.DisplayName);
            root.SetDiagnosticActivityTags(Activity.Current);
            return Next.ScheduleActivityAsync<TResult>(input with { Headers = root.HeaderFromContext(input.Headers) });
        }
    ...
    private class ContextPropagationActivityInboundInterceptor : ActivityInboundInterceptor
    {
        ...
        {
            return root.WithHeadersApplied(input.Headers, () =>
            {
                root.SetDiagnosticActivityTags(Activity.Current);
